...
$turma_pesq2 = mysql_query("SELECT cod_turma, grupo, nivel, curso, modulo, unidade, polo, inicio, fim, tipo_etapa, encerrado FROM ced_turma WHERE id_turma = $id_turma");
                  ...
                  <div class="col-md-12">
                      <section class="panel">
                          <header class="panel-heading">
                              <a href="p_listar_aulas.php?id=<?php echo $turma_d?>&id_turma=<?php echo $id_turma;?>"><span class="label label-inverse">Registro de Frequ&ecirc;ncia</span></a>
...
<table width="100%" border="1" class="full_table_list" style="font-size:10px; font-family:Arial, Helvetica, sans-serif;">
    ...
    <tr>
    <th colspan="2"><img src="../acesso/images/logo-cedtec.png" /></th>
...
WHERE oco.n_ocorrencia = 1 AND oco.matricula = $codigo AND oco.id_turma = $id_turma LIMIT 1");
	...
	AND cta.id_etapa = $etapa_id");
						...
						if($etapa_id != 3&&$grupo_ativi != "C"&&$etapa_id != 5){
							echo "
							<td bgcolor=\"$etapa_cor\" align=\"center\"><a target=\"blank\" href=\"lancar_nota2.php?td=$turma_d&etapa=$etapa_id\">$nota_aluno</a></b></td>";
						}
						if($etapa_id != 3&&$grupo_ativi == "C"&&$etapa_id != 5){
							echo "
							<td bgcolor=\"$etapa_cor\" align=\"center\"><a target=\"blank\" href=\"lancar_nota2.php?td=$turma_d&etapa=$etapa_id\">$nota_aluno</a></b></td>";
