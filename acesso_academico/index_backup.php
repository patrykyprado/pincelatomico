...
$exibir_professor_topo
		...
		while($dados_etapa = mysql_fetch_array($sql_etapa_notas)){
			...
			while($dados_atividades = mysql_fetch_array($sql_atividades)){
				...
				SELECT SUM(nota) AS notafinal FROM view_boletim_aluno WHERE matricula = $user_usuario AND turma_disc = $turma_disc
				AND subgrupo LIKE '%$atividade_grupo%' AND id_etapa = $etapa_id");
