...
$exibir_professor_topo
		...
		while($dados_etapa = mysql_fetch_array($sql_etapa_notas)){
			...
			$ativs_etapa = $dados_etapa["grupos_ativ"];
			...
			while($dados_atividades_etapa = mysql_fetch_array($sql_atividades_etapa_notas)){
				...
				WHERE cn.matricula = $user_usuario AND cn.turma_disc = $turma_disc AND cdn.subgrupo = '$atividades_etapa_grupo'  AND cn.ref_ativ IN (SELECT ref_id FROM ced_turma_ativ)
				...
				if($contar_nota == 0){
					...
					$nota_aluno = format_valor($dados_nota["notafinal"]);
					$nota_parcial += $dados_nota["notafinal"];
				}
		...
		a_detalhes_nota.php?matricula=$user_usuario&td=$turma_disc&grupo=$atividades_etapa_grupo&etapa=$etapa_id\">$nota_aluno</a></td>
				";
			}
			echo "
		...
		if(str_replace(",",".",$exibir_nota_final) < str_replace(",",".",$etapa_min_nota)){
				$cor_nota_final = "red";	
