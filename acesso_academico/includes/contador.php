<?php
session_start();
$expira_em  = 120; //DEFINE EM MINUTOS A EXPIRAÇÃO DO ACESSO DO USUARIO
$sessao     = session_id();
...
while($dados_desconectar = mysql_fetch_array($sql_desconectar)){
	...
	mysql_query("UPDATE acesso SET chat_status = 1 WHERE usuario = '$usuario_desconectar'");
}
...
VALUES('$sessao','$user_empresa','$user_usuario','$user_nivel','$user_unidade','$user_email','$user_acessos','$user_nome','$user_senha','$tempo_on','$ip','$acesso_pagina_atual')");
  ...
  mysql_query("UPDATE acessos SET chat_status = 2 WHERE codigo = $user_usuario");
  if($total_c){
