  ...
  <table width="100%" border="1" align="center" class="full_table_list2">
	...
	$total = mysql_num_rows($pesquisar);
		while($dados_ativ = mysql_fetch_array($pesquisar)){
			...
			$sql_etapa = mysql_query("SELECT * FROM ced_etapas WHERE id_etapa = $etapa_ativ");
			...
			echo "<td><a href=\"editar_atividade.php?id_atividade=$ref_id&turma=$turma_disc\" title=\"$desc_ativ\"><center><b>$nome_atividade<br><font size='-1'>($data_ativ)</font><br><font size='-1'>($nome_etapa)</font></b></a><br><font-size='-2'><a href=\"javascript:aviso('$ref_id')\">[Excluir]</font></a></center></td>";
			}
...
<!--
function aviso(id){
if(confirm ('Todas as notas serão apagadas. Deseja realmente excluir a atividade?'))
{
location.href="excluir_atividade.php?id="+id;
}
else
{
return false;
