...
echo "<div class=\"filtro\"><center><a href=\"javascript:window.print();\">[IMPRIMIR]</a></center></div>";
$id = $_GET["codigo"];
...
while($dados_aluno = mysql_fetch_array($sql_alunos)){
	...
	$sql_etapa_atividades = mysql_query("SELECT * FROM ced_etapas WHERE tipo_etapa LIKE '%$tipo_etapa%'");
	$sql_etapa_nome = mysql_query("SELECT * FROM ced_etapas WHERE tipo_etapa LIKE '%$tipo_etapa%'");
...
AND cta.id_etapa = $etapa_id");
		...
		if($nota_final >= 60){
			...
			$exibir_resultado = "<td align=\"center\" bgcolor=\"#FFDAB9\"><b><div style=\"color:#000\" id=\"resultado\">Recuperação</div></b></td>";
		}
