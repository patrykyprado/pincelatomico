...
if($contar_agrupamento == 1){
	...
	for( $i = 0 , $x = count( $turmas_exp ) ; $i <= $x ; ++ $i ) {
		...
		if(@mysql_query("INSERT INTO ea_forum (id_forum,cod_ativ,turma_disc,subturma,data_criacao,data_inicio,data_fim,senha_acesso,max_nota,resposta,titulo,descricao) VALUES (NULL,'$codativ','$turma_disc','$id_agrupamento','$data_criacao','$datahora_inicio','$datahora_final','$senha_acesso','$nota','$resposta','$titulo','$descricao')")){
			...
			VALUES (NULL, '$turma_disc', 'B', '17', '$nota', '$data_boletim', 'Fórum - $titulo', '$id_atividade_final','4')");
				}
			}
...
if(@mysql_query("INSERT INTO ea_forum (id_forum,cod_ativ,turma_disc,subturma,data_criacao,data_inicio,data_fim,senha_acesso,max_nota,resposta,titulo,descricao) VALUES (NULL,'$codativ','$turma_disc','0','$data_criacao','$datahora_inicio','$datahora_final','$senha_acesso','$nota','$resposta','$titulo','$descricao')")){
	if(mysql_affected_rows() ==1){
		...
		$sql_p_forum = mysql_query("SELECT MAX(id_forum) as id_atividade FROM ea_forum WHERE cod_ativ LIKE '$codativ' AND turma_disc LIKE '$turma_disc'");
		$dados_p_forum = mysql_fetch_array($sql_p_forum);
		$id_atividade = trim($dados_p_forum["id_atividade"]);
		$id_atividade_final = "F_".$id_atividade;
		if($nota != 0){
			mysql_query("INSERT INTO ced_turma_ativ (ref_id, cod_turma_d, grupo_ativ, cod_ativ, valor, data, descricao, id_atividade, id_etapa)
	VALUES (NULL, '$turma_disc', 'B', '17', '$nota', '$data_boletim', 'Fórum - $titulo', '$id_atividade_final', '4')");
                  ...
                  <div class="col-md-12">
                      <section class="panel">
                          <header class="panel-heading">
                              <b>Cadastro de T&oacute;picos</b>
                          </header>
