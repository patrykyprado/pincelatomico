...
SELECT DISTINCT matricula, nome FROM view_boletim_alunos
WHERE id_turma = $id_turma ORDER BY alu.nome");
} else {
$sql_alunos = mysql_query("
SELECT DISTINCT matricula, nome FROM view_boletim_alunos
WHERE id_turma = $id_turma AND matricula = $id_aluno ORDER BY nome");
}
...
?>
  ...
  <body>
  <div class="filtro">
<center><a href="javascript:window.print();">[IMPRIMIR BOLETIM]</a></center></div>
...
<td bgcolor=\"#CCC\" align=\"center\"><b>Resultado</b></td></tr>";
		...
		while($dados_etapa = mysql_fetch_array($sql_etapa_notas)){
			...
			SELECT notafinal FROM view_boletim_notas
WHERE matricula = $mat_aluno AND turma_disc = $turma_disc AND subgrupo = 'A' AND id_etapa = $etapa_id");
			...
			SELECT notafinal FROM view_boletim_notas
WHERE matricula = $mat_aluno AND turma_disc = $turma_disc AND subgrupo = 'B'  AND id_etapa = $etapa_id");
			...
			if($etapa_id != 3&&$etapa_id !=5){
				...
				SELECT notafinal FROM view_boletim_notas
	WHERE matricula = $mat_aluno AND turma_disc = $turma_disc AND subgrupo = 'C'  AND id_etapa = $etapa_id");
