...
INNER JOIN ced_turma B ON A.id_turma = B.id_turma WHERE B.grupo LIKE '$grupo' AND B.modulo = '$modulo' AND B.turno LIKE '%$turno%' AND B.id_turma = '$id_turma')  AND codigo = '$matricula'");
...
if ($desenturmado == 0) {
    ...
    while (($dados = mysql_fetch_array($sql2))) {
		...
		echo ("<SCRIPT LANGUAGE='JavaScript'>
			window.alert('Aluno(a) $nome foi inserido na turma $turma');
