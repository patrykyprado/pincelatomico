...
if($transacao == "="){
	$transacao2 = " OR data_pagto IS NULL";	
} else {
	$transacao2 = "";
}
//TODAS UNIDADES
...
if($unidade=="*"&&$transacao <>"*"&&$conta =="*"){
	$sql = mysql_query("SELECT * FROM geral_titulos WHERE $sql_filtro OR (vencimento BETWEEN '$inicio' AND '$fim')) AND tipo_titulo = 1 AND (data_pagto $transacao '' $transacao2) AND empresa LIKE '$empresa' $comp_sql $sql_order");
	$despesa = mysql_query("SELECT SUM( valor ) as despesapagar FROM geral_titulos WHERE (vencimento BETWEEN '$inicio' AND '$fim') AND tipo_titulo = 1 AND (valor_pagto = 0 OR data_pagto ='') AND empresa LIKE '$empresa' $comp_sql");
...
if($unidade=="*"&&$transacao <>"*"&&$conta <>"*"){
	$sql = mysql_query("SELECT * FROM geral_titulos WHERE $sql_filtro OR (vencimento BETWEEN '$inicio' AND '$fim')) AND tipo_titulo = 1 AND conta LIKE '$conta' AND (data_pagto $transacao '' $transacao2) AND empresa LIKE '$empresa' $comp_sql $sql_order");
...
if($unidade<>"*"&&$transacao <>"*"&&$conta =="*"){
	$sql = mysql_query("SELECT * FROM geral_titulos WHERE $sql_filtro AND tipo_titulo = 1 AND (data_pagto $transacao '' $transacao2) AND conta_nome LIKE '%$unidade%' AND empresa LIKE '$empresa' $sql_order");
	$despesa = mysql_query("SELECT SUM( valor ) as despesapagar FROM geral_titulos WHERE (vencimento BETWEEN '$inicio' AND '$fim') AND tipo_titulo = 1 AND (valor_pagto = 0 OR data_pagto ='') AND conta_nome LIKE '%$unidade%' AND empresa LIKE '$empresa'");
...
if($unidade<>"*"&&$transacao <>"*"&&$conta <>"*"){
	$sql = mysql_query("SELECT * FROM geral_titulos WHERE $sql_filtro AND tipo_titulo = 1 AND conta LIKE '$conta' AND (data_pagto $transacao '' $transacao2) AND conta_nome LIKE '%$unidade%' AND empresa LIKE '$empresa' $sql_order");
	$despesa = mysql_query("SELECT SUM( valor ) as despesapagar FROM geral_titulos WHERE (vencimento BETWEEN '$inicio' AND '$fim') AND conta LIKE '$conta' AND tipo_titulo = 1 AND (valor_pagto = 0 OR data_pagto ='') AND conta_nome LIKE '%$unidade%' AND empresa LIKE '$empresa'");
